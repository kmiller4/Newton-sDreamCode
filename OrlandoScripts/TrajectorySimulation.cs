...
public class TrajectorySimulation : MonoBehaviour
{
	...
	void simulatePath()
	{
		...
		Vector3 segVelocity = GetComponent<Rigidbody>().velocity * 250 * Time.deltaTime;
		...
		for (int i = 1; i < segmentCount; i++)
		{
			...
			if (Physics.Raycast(segments[i - 1], segVelocity, out hit, segmentScale))
			{
				...
				segVelocity = segVelocity * (segmentScale - hit.distance) / segVelocity.magnitude;
		...
		sightLine.SetWidth (0.04f, 0.04f);
